...
THREE.MeasurementGizmo = function ( measurement, container ) {
	...
	this.controlPoints = [];
	this.annoContent = "none";
	var projector = new THREE.Projector();
	this.getContent = function(){
		return scope.annoContent;
	}
	...
	this.changePointAndObject = function(name,point,object){
		...
		switch (name) {
			...
			case "STARTARROW":
				...
				{				
					if(object){
						var point1 = new THREE.Vector3().copy(point);
						var point2 = new THREE.Vector3().copy(point);
						var point3 = new THREE.Vector3().copy(this.controlPoints[1].point);
						this.controlPoints[1].object.localToWorld(point3);
						var point4 = new THREE.Vector3().copy(this.controlPoints[2].point);
						this.controlPoints[2].object.localToWorld(point4);
						object.worldToLocal(point1);
						object.worldToLocal(point2);
						object.worldToLocal(point3);
						object.worldToLocal(point4);
						this.controlPoints[0].point = point1;
						this.controlPoints[0].object = object;
						this.controlPoints[3].object = object;
						this.controlPoints[3].point = point2;
						if(scope.measurement.getType() == "Annotate"){
							this.controlPoints[1].object = object;
							this.controlPoints[1].point = point3;
							this.controlPoints[2].object = object;
							this.controlPoints[2].point = point4;
							console.log("change");
						}  
						object.add(this.measurement);
					}else{
						var offset = controlPoints[3].sub(controlPoints[0]).negate()
			...
			case "ENDARROW":
				if (this.controlPoints.length == 4)
				{
					if(object){
						...
						object.worldToLocal(point1);
						object.worldToLocal(point2);
						this.controlPoints[1].point = point1;
						this.controlPoints[1].object = object;
						this.controlPoints[2].object = object;
						this.controlPoints[2].point = point2;  
						object.add(this.measurement);
					}else{
						var offset = controlPoints[2].sub(controlPoints[1]).negate()
