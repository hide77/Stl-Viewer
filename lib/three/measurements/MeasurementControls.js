...
THREE.MeasurementControls = function ( viewport, camera, container ) {
		...
		if ( pointer.button === 0 || pointer.button == undefined ) {
			...
			if (measurementGizmo && measurementGizmo.acceptPoints()) {
				...
				if (intersect) {
					...
					if(measurementGizmo.measurement.getType() == "Annotate"){
						AnnotateModal(event,measurementGizmo);
					}
	...
	function onPointerUp( event ) {
		...
		//	event.cancel = true; //prevent other listeners from getting this event
		var pointer = event.touches? event.touches[0] : event;
		var intersect = intersectMouseUp( pointer, viewport.objects, true );

		var measurementGizmo = scope.selectedPicker ? scope.selectedPicker.measurementGizmo : null;
		if(scope.dragging&&measurementGizmo){
			measurementGizmo.changePointAndObject(scope.selectedPicker.name,intersect.point,intersect.object);
		}
	...
	function intersectObjects( pointer, objects, recursive ) {
		...
		{
			while (intersections.length > 0 &&((intersections[0].object.type === "LineSegments"))
	...
	function intersectMouseUp(pointer, objects, recursive){
		ray.set( camera.position, getEyeVector( pointer ));
		if (!objects) {
			console.log('err');
		}
		...
		if (intersections.length > 0)
		{
			while (intersections.length > 0 &&((intersections[0].object.type === "LineSegments"))
				...
				|| (!intersections[0].object.measurementGizmo && intersections[0].object.visible === false)))
				intersections.shift();
			while(intersections.length>0&&(intersections[0].object.name !== intersections[0].object.uuid||intersections[0].object.name === "STARTARROW"||intersections[0].object.name === "STARTLINE"||intersections[0].object.name === "ENDARROW"||intersections[0].object.name === "ENDARROW"))
				intersections.shift();
		}		
		return intersections[0] ? intersections[0] : false;
	}
