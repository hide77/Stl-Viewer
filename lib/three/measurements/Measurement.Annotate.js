...
THREE.MeasurementAnnotate = function ( ) {

	THREE.Measurement.call( this );
	
	this.createGizmo = function(container) {
		this.measurementGizmo = new THREE.MeasurementGizmoAnnotate( this, container );
...
THREE.MeasurementAnnotate.prototype = Object.create( THREE.Measurement.prototype );
...
THREE.MeasurementGizmoAnnotate = function ( measurement, container ) {
	...
	THREE.MeasurementGizmoAnnotate.prototype.setContent = function(str){
		scope.annoContent = str;
	}
	THREE.MeasurementGizmoAnnotate.prototype.addControlPoint = function(point, object, forceAdd, face, callbackAddedObject) {
	...
	THREE.MeasurementGizmoAnnotate.prototype.updateGizmosFromControlPoints = function(camera) {
		var scope = this;
		//getting width
		...
		switch (scope.controlPoints.length) {
			...
			case 2:
				...
				//text picker
				var midPoint = new THREE.Vector3().copy(topPoints[1]);
				var object = scope.pickerGizmos['TEXT'][0];
				object.position.copy(midPoint);
				object.scale.set(width, width, width);

				//set text value
				scope.setText(scope.getContent(), midPoint, camera);
...
THREE.MeasurementGizmoAnnotate.prototype = Object.create( THREE.MeasurementGizmo.prototype );